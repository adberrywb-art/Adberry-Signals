...
async function startServer() {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('disconnect', (reason) => console.log(`🔌 [SOCKET] Cliente desconectado (${socket.id}): ${reason}`));
  });
  ...
  bot.on('message', (msg) => {
    ...
    // Procesar señal si el mensaje existe (relajamos la restricción de Chat ID para pruebas)
    if (msg.text) {
  ...
  const processSignalForAllUsers = async (text: string) => {
    ...
    if (type) {
      ...
      for (const user of users) {
        ...
        console.log(`📡 Emitiendo señal a Socket.io para usuario ${user.id}:`, newSignal.type, newSignal.pair);
  ...
  app.post('/api/signals/simulate', (req, res) => {
    const { text } = req.body;
    console.log(`🧪 [SERVER] Petición de simulación: "${text}"`);
    processSignalForAllUsers(text || 'BUY EURUSD SL 1.0800 TP 1.0900');
    res.json({ success: true });
  });
  ...
  app.get('/api/mt5/pending/:account', (req, res) => {
    ...
    console.log(`🔍 MT5 Polling: Cuenta ${account}`);
    let user = db.prepare('SELECT id, lot_size FROM users WHERE mt5_account = ?').get(account);
    
    if (!user) {
      console.log(`⚠️ MT5 Polling: Cuenta ${account} no encontrada. Creando cuenta temporal...`);
      try {
        const info = db.prepare('INSERT INTO users (username, password, mt5_account, lot_size) VALUES (?, ?, ?, ?)')
          .run(`user_${account}`, 'temp_pass', account, 0.01);
        user = { id: Number(info.lastInsertRowid), lot_size: 0.01 };
      } catch (e) {
        // Si ya existe por username pero no por mt5_account (raro pero posible)
        user = db.prepare('SELECT id, lot_size FROM users WHERE username = ?').get(`user_${account}`);
      }
    }

    if (!user) {
      return res.status(404).json({ error: 'No se pudo crear/encontrar cuenta' });
    }

    const pendingSignals = db.prepare('SELECT * FROM signals WHERE user_id = ? AND executed_on_mt5 = 0').all(user.id);
    console.log(`✅ MT5 Polling: ${pendingSignals.length} señales pendientes para ${account}`);
  ...
  app.post('/api/auth/register', async (req, res) => {
    ...
    try {
      ...
      res.json({ id: Number(info.lastInsertRowid), username, capital: 1000, lot_size: 0.01, success_rate: 0 });
    } catch (e) {
