...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on('new_signal', (signal: Signal) => {
      console.log('🔔 [SOCKET] Nueva señal recibida:', signal);
                  ...
                  <div className="bg-[#0F172A]/40 backdrop-blur-md border border-white/5 p-6 rounded-3xl shadow-2xl flex flex-col">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="flex items-center gap-3">
                        <button 
                          onClick={() => fetchData()}
                          className="px-2 py-1 bg-slate-900 border border-white/5 rounded text-[8px] font-black text-slate-500 hover:text-emerald-500 transition-all uppercase tracking-widest"
                        >
                          Refrescar
                        </button>
                        <button 
                          onClick={async () => {
                            const text = prompt('Escribe el mensaje de la señal (ej: BUY BTCUSD SL 60000 TP 70000)', 'BUY BTCUSD SL 60000 TP 70000');
                            if (text) {
                              console.log('🧪 [CLIENT] Enviando simulación:', text);
                              ...
                              if (res.ok) {
                                console.log('✅ [CLIENT] Simulación enviada al servidor');
                              } else {
                                console.error('❌ [CLIENT] Error al enviar simulación:', res.status);
                              }
                            }
                          }}
                          className="px-2 py-1 bg-rose-500/10 border border-rose-500/20 rounded text-[8px] font-black text-rose-500 hover:bg-rose-500/20 transition-all uppercase tracking-widest"
                        >
                          Simular
                        </button>
                        <button 
                        ...
                        <div className="bg-slate-900 p-4 rounded-xl border border-white/5 relative group">
                          <div className="mb-4 space-y-2">
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">
                              ⚠️ URL DE ESTE SERVIDOR (Copia esto en tu script):
                            </p>
                            <code className="bg-black/40 px-2 py-1 rounded text-emerald-400 text-[10px] break-all block border border-white/5">
                              {window.location.origin}
                            </code>
                            <p className="text-rose-400 text-[9px] font-medium italic">
                              * Si usas otro URL (como Railway), el script no funcionará con esta sesión.
                            </p>
                          </div>
...
while True:
    try:
        url = f"{API_URL}/api/mt5/pending/{ACCOUNT_ID}"
        ...
        if res.status_code == 200:
            try:
                ...
                if signals:
                    print(f"📡 Recibidas {len(signals)} señales")
                for s in signals:
                ...
                print(f"❌ Error: El servidor no devolvió JSON válido. Respuesta: {res.text[:100]}")
        elif res.status_code == 404:
            print(f"⚠️ Error 404: Cuenta {ACCOUNT_ID} no configurada en la web. Ve a Ajustes y dale a 'Aplicar'.")
            time.sleep(10)
        else:
